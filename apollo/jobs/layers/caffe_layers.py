...
class Lstm(Layer):
    def __init__(self, name, num_cells, weight_filler=None, **kwargs):
        super(Lstm, self).__init__(self, name, kwargs)
        self.p.lstm_param.num_cells = num_cells
        if weight_filler is None:
            weight_filler = Filler('uniform', 0.1)
        self.p.lstm_param.input_weight_filler.CopyFrom(weight_filler.filler_param)
        self.p.lstm_param.input_gate_weight_filler.CopyFrom(weight_filler.filler_param)
        self.p.lstm_param.forget_gate_weight_filler.CopyFrom(weight_filler.filler_param)
        self.p.lstm_param.output_gate_weight_filler.CopyFrom(weight_filler.filler_param)
...
class NumpyData(DataLayer):
    def __init__(self, name, data, **kwargs):
        ...
        from apollo import make_numpy_data_param
        import numpy as np
